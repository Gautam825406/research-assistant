"""Streamlit frontend entrypoint for the AI Research Assistant.
...
def render_home() -> None:
    ...
    if health and "app_name" in health and "version" in health:
        ...
        st.warning(
            f"A server responded at `{API_BASE_URL}` but not with the expected AI Research "
            "Assistant API schema. Is another service running on this port?"
        )
    else:
        st.error(
